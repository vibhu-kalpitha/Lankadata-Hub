    ...
    <div className="flex-1 bg-lanka-bg min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 flex flex-col lg:flex-row gap-8">
        ...
        <aside className="w-full lg:w-64 flex-shrink-0">
          <div className="sticky top-24 bg-[#050d1a] border border-lanka-border rounded-2xl p-5 space-y-6">
                ...
                {categories.map(cat => (
                  <label key={cat.id} className="flex items-center justify-between cursor-pointer group">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${selectedCategory.toLowerCase() === cat.id ? 'bg-blue-600 border-blue-500' : 'bg-white/5 border-lanka-border group-hover:border-lanka-border-hover'}`}
                    ...
                    <span className="text-[10px] bg-white/5 border border-lanka-border px-1.5 py-0.5 rounded-lg font-mono text-lanka-darkText">{cat.count}</span>
                  </label>
                ))}
              </div>
            </div>
                ...
                {['CSV', 'JSON', 'API', 'Excel'].map(fmt => {
                  ...
                  return (
                    <button key={fmt} onClick={() => setSelectedFormat(p => p.toLowerCase() === fmt.toLowerCase() ? '' : fmt)}
                      className={`text-[10px] font-bold px-3 py-1.5 rounded-xl border transition-all ${active ? 'bg-blue-600/30 border-blue-500 text-cyan-300 shadow-[0_0_12px_rgba(37,99,235,0.4)]' : 'bg-white/5 border-lanka-border text-lanka-muted hover:border-lanka-border-hover hover:text-white'}`}>
                      {fmt}
                    </button>
                  );
                })}
        ...
        <section className="flex-1 space-y-6">
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search datasets (e.g. GDP, Fuel prices, Census)..."
                ...
                className="w-full bg-[#050d1a] border border-lanka-border hover:border-lanka-border-hover focus:border-blue-500 rounded-xl py-3 pl-11 pr-4 text-xs text-white placeholder-lanka-darkText focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors" />
            </div>
            <div className="flex items-center gap-2 bg-[#050d1a] border border-lanka-border rounded-xl px-3 py-1.5">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-[#050d1a] border border-lanka-border rounded-2xl">
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 pt-4">
              <button onClick={() => handlePage(currentPage - 1)} disabled={currentPage === 1}
                className="p-2 rounded-xl border border-lanka-border bg-white/5 hover:bg-white/10 text-white disabled:opacity-30 transition-colors">
                <ChevronLeft size={14} />
              </button>
              {Array.from({ length: totalPages }).map((_, i) => (
                <button key={i} onClick={() => handlePage(i + 1)}
                  className={`px-3 py-1.5 text-xs font-bold rounded-xl border transition-all ${i + 1 === currentPage ? 'bg-blue-600 border-blue-500 text-white shadow-[0_0_12px_rgba(37,99,235,0.5)]' : 'bg-white/5 border-lanka-border text-lanka-muted hover:border-lanka-border-hover hover:text-white'}`}>
                  {i + 1}
                </button>
              ))}
